...
<div class="ml-64 p-8">
  ...
  <!-- FORM -->
  <div class="bg-white dark:bg-[#111] p-8 rounded-3xl shadow-xl mb-8
    border border-gray-100 dark:border-[#1f1f1f]">

    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">

      <input type="number" id="listrik" placeholder="Biaya Listrik" class="input">
      <input type="number" id="air" placeholder="Biaya Air" class="input">
      <input type="number" id="wifi" placeholder="Biaya Wifi" class="input">
      <input type="number" id="sampah" placeholder="Biaya Sampah" class="input">

      <!-- JUMLAH PENGHUNI -->
      <input type="number" id="penghuni" placeholder="Jumlah penghuni aktif" class="input">
    ...
    <div class="mt-6 bg-blue-50 dark:bg-[#0b2239] p-6 rounded-2xl">
      ...
      <h3 id="perOrang" class="text-xl font-semibold text-green-600 mt-1">
        Rp 0
      </h3>
    </div>
    ...
    <div class="flex justify-end gap-3 mt-6">
      <button onclick="resetForm()"
        class="px-6 py-3 rounded-xl bg-gray-200 dark:bg-[#1a1a1a]">
        Reset
      </button>
  ...
  <!-- LIST TAGIHAN -->
  <div class="bg-white dark:bg-[#111] p-6 rounded-3xl shadow-xl
    border border-gray-100 dark:border-[#1f1f1f]">
...
const inputs = document.querySelectorAll('#listrik, #air, #wifi, #sampah, #penghuni');
...
function hitungTotal() {
  ...
  total = listrik + air + wifi + sampah;

  totalText.innerText = formatRupiah(total);

  if (penghuni > 0) {
    const perOrang = total / penghuni;
    perOrangText.innerText = formatRupiah(Math.round(perOrang));
  } else {
...
function tambahTagihan() {
  ...
  const row = `
    <tr class="border-t border-gray-100 dark:border-[#222]">
      <td class="py-4">Bulan Baru</td>
      <td>${formatRupiah(total)}</td>
      <td>${formatRupiah(Math.round(perOrang))}</td>
      <td>
